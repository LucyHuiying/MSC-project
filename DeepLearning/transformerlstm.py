...
class LSTMTransformer(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, d_model, nhead, num_encoder_layers):
        super(LSTMTransformer, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)
        self.linear = nn.Linear(hidden_size, d_model)  # 将 LSTM 输出调整到 Transformer 的 d_model 个通道
        self.transformer = TransformerEncoder(d_model=d_model, nhead=nhead, num_layers=num_encoder_layers)
        self.fc = nn.Linear(d_model + hidden_size, num_classes)
...
model = LSTMTransformer(input_size, hidden_size, num_layers, num_classes, d_model, nhead, num_encoder_layers).to(device)
